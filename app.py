...
if st.session_state.avg_daily_volume == 0 and price_levels > 0 and threshold > 0:
    avg_volume = fetch_avg_daily_volume_sync(SYMBOL, DAYS_AVG_VOLUME)
...
while not st.session_state.data_queue.empty():
    ...
    if 'error' in data:
        ...
        st.session_state.liquidity_bid = data['liquidity_bid']
        st.session_state.liquidity_ask = data['liquidity_ask']
        st.session_state.liquidity_bid_pct = data['liquidity_bid_pct']
        st.session_state.liquidity_ask_pct = data['liquidity_ask_pct']
